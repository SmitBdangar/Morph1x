"""
Utility functions for video processing, drawing, and general helpers.
"""
...
def draw_detection_box(frame: np.ndarray, detection: Dict, 
    ...
    Returns:
        Frame with drawn detection box and label
    """
    if color is None:
        color = BOX_COLOR
...
def get_video_source_info(source) -> Dict:
    ...
    Args:
        source: Video source (camera index, file path, etc.)
...
def create_test_frame(width: int = 640, height: int = 480) -> np.ndarray:
    """
    Create a test frame for debugging purposes.
    ...
    Returns:
        Test frame with some basic shapes
    """
